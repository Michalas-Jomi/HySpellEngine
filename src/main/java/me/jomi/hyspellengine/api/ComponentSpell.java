...
public abstract class ComponentSpell<C extends Component<EntityStore>> extends Spell {
    private final ComponentType<EntityStore, C> componentType;

    protected ComponentSpell(String name, String description, ComponentType<EntityStore, C> componentType) {
        super(name, description);
        this.componentType = componentType;
    }

    public abstract void apply(SpellContext context, Ref<EntityStore> ref, Store<EntityStore> store, C component);

    @Override
    public final void apply(SpellContext context, Ref<EntityStore> ref, Store<EntityStore> store) {
        this.apply(context, ref, store, store.ensureAndGetComponent(ref, this.getComponentType()));
    }
